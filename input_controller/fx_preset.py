...
class FxPreset(IFxPresetDataProvider):
    ...
    def __init__(self, context, fx_page_number, fx_number, view, daw_fx_manager):
    ...
    def view_update_fx_preset_availability(self):
        self.__view.set_fx_preset_availability(self.__fx_number, len(self.__persistency_item.get_mixer_parameters()) > 0)
    ...
    def __are_parameters_loaded(self):
        return len(self.__persistency_item.get_mixer_parameters()) != 0

    def __get_params_from_plugins(self):
        self.__persistency_item.reset_mixer_parameters()
        self.__persistency_item.set_mixer_parameters(self.__daw_fx_manager.get_parameters())

    def __apply_parameters_to_plugins(self):
        mixer_parameters = self.__persistency_item.get_mixer_parameters()
        self.__daw_fx_manager.set_parameters(mixer_parameters)
